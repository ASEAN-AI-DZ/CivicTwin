...
export default function AdminSidebar() {
  ...
  return (
    <aside className="fixed inset-y-0 left-0 z-50 w-[72px] lg:w-[260px] bg-card/95 backdrop-blur-3xl border-r border-border transition-all duration-300 flex flex-col items-center lg:items-stretch shadow-2xl">
      {/* Brand */}
      <div className="h-20 flex items-center justify-center lg:justify-start lg:px-6 border-b border-border bg-muted/30">
        <div className="flex items-center justify-center relative shrink-0 group">
          <Image src="/logo.png" alt="CivicTwin AI Logo" width={40} height={40} className="relative z-10 object-contain w-10 h-10 drop-shadow-[0_0_10px_rgba(20,184,166,0.3)] transition-transform duration-300 group-hover:scale-110" unoptimized />
        </div>
        <div className="hidden lg:flex flex-col ml-3">
          ...
          <span className="text-[10px] font-bold text-teal-500 uppercase tracking-[0.2em] mt-0.5">City Admin</span>
        </div>
      </div>
      {/* Navigation */}
          ...
          return (
            <Link
              ...
              className={`group flex items-center justify-center lg:justify-start gap-3 p-3 lg:px-4 lg:py-3.5 rounded-xl transition-all duration-200 relative overflow-hidden ${
                isActive 
                  ? 'nav-active-bar bg-teal-500/8 text-teal-500 border border-teal-500/20 shadow-[0_0_10px_rgba(20,184,166,0.1)]' 
                  : 'text-muted-foreground hover:bg-accent hover:text-accent-foreground border border-transparent'
              }`}
            >
              <item.icon className={`w-5 h-5 flex-shrink-0 transition-transform ${isActive ? '' : 'group-hover:translate-x-0.5'}`} />
